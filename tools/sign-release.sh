...
#
# Парольная фраза передаётся только окружением: BSDNAS_PASSFILE указывает
# на файл с фразой. Где живут ключ и файл — не документируется.

set -eu

KEYID="${BSDNAS_SIGNKEY:-1E2F7A792066322D}"
PASSFILE="${BSDNAS_PASSFILE:?задайте BSDNAS_PASSFILE (файл с парольной фразой)}"
DIR="${1:-}"
